...
import { Activity, BarChart3, Clock, RefreshCw, AlertTriangle } from 'lucide-react'
...
export function MonitorPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const params = new URLSearchParams({ since_minutes: String(sinceMinutes), per_page: '100' })
      ...
      // silently fail if project not yet synced
    } finally {
      setLoading(false)
    }
  ...
  const statusColor = (code: number) => {
    ...
    if (code >= 300) return 'var(--accent-blue)'
    return 'var(--accent-green)'
  }

  const methodColor = (method: string) => {
    const map: Record<string, string> = {
      GET: 'var(--accent-green)', POST: 'var(--accent-blue)',
      PUT: '#f59e0b', PATCH: '#f59e0b', DELETE: 'var(--accent-red)',
    }
    ...
    <div className="page-container">
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
          ...
          <button type="button" className="btn ghost" onClick={fetchData} disabled={loading} style={{ padding: '0.35rem 0.6rem' }}>
